    ...
    TrelloCSVExport.prototype.board_to_csv = function() {
      ...
      return Trello.boards.get("" + this.board.id + "/lists", {
        ...
        $.each(lists, function(index, list) {
          _this.csv_string += _this.line_ending;
          _this.csv_string += "" + list.name + _this.line_ending;
          return $.each(list.cards, function(index, card) {
            return _this.csv_string += "" + card.name + _this.line_ending;
          });
        });
        return $('<form></form>', {
          action: "http://savefile.joshmcarthur.com/" + _this.board.name + ".csv",
          method: 'post'
        }).append($('<input></input>', {
          type: 'hidden',
          name: 'content',
